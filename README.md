...
1. **Submit a Query:**
   - A client can send a chat message either to a RabbitMQ queue (default: `chat_incoming`) **or** via the `/v1/chat/completions` REST API endpoint.
...
2. **Processing the Query:**
   - The worker consumes messages from the input queue or receives them via the REST API.
...
3. **Returning the Response:**
   - If the request was sent via the REST API, the response is streamed back to the client in OpenAI-compatible SSE format.
   - If the request was sent via RabbitMQ, the worker publishes the LLM's response to a dedicated RabbitMQ queue named `{RESPONSE_QUEUE_PREFIX}{uid}` (e.g., `response_123e4567-e89b-12d3-a456-426614174000`).
...
| `RABBITMQ_QUEUE`         | Name of the input queue for chat requests        | `chat_incoming`                         |
| `RESPONSE_QUEUE_PREFIX`  | Prefix for the response queue                    | `response_`                             |
| `RESPONSE_QUEUE_RETRIES` | Number of attempts to connect to response queue  | `300`                                   |
...
Consume from the queue named `{RESPONSE_QUEUE_PREFIX}{uid}` (replace `{uid}` with the UID you used, and the prefix with your configuration):
...
The application exposes a REST API compatible with the OpenAI Chat Completions endpoint. When you submit a chat query to `/v1/chat/completions`, the API will stream the response back to you as it becomes available, using OpenAI-compatible SSE (Server-Sent Events) format.
...
- Each chunk of the LLM's response will be sent as a new SSE event, with a JSON payload compatible with OpenAI's streaming API (see their docs for details).
